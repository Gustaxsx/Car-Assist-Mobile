...
fun TransferenciaConfirmarScreen(
    ...
    // 💡 Captura o contexto para exibir o Toast
    val context = LocalContext.current

    // Ouve o evento de sucesso para navegar para a próxima tela ou mostrar erro
    LaunchedEffect(Unit) {
        viewModel.eventFlow.collect { event ->
            when (event) {
                is TransferenciaUiEvent.Sucesso -> {
                    navController.navigate("transferencia_codigo")
                }
                is TransferenciaUiEvent.Erro -> {
                    // 💡 Dispara o pop-up (Toast) com a mensagem de erro
        ...
        Box(
            ...
            Column(
                ...
                Row(
                    ...
                    Button(
                        onClick = {
                            viewModel.gerarCodigoTransferencia(idUsuarioLogado) // Chama a API
                        },
                        modifier = Modifier
                            .weight(1f)
                            .height(52.dp),
